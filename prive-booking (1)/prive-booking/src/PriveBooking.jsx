...
const EXTRAS = [
  ...
  { key: "transport", label: "Home Service (Transport)", price: 3000 },
];
...
export default function PriveBooking() {
  ...
  const [extras, setExtras] = useState({ extraLength: false, boho: false, beads: false, transport: false });
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 28 }}>
              {SERVICES.map(s => (
                <div key={s.id}
                  ...
                  <div style={{ color: TEXT_MUTED, fontSize: 11, lineHeight: 1.5, marginBottom: 8, fontWeight: 500 }}>{s.desc}</div>
                  <div style={{ color: GOLD, fontSize: 10, letterSpacing: 1, fontWeight: 600 }}>⏱ {s.duration}</div>
                </div>
              ))}
            </div>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {Object.entries(availableSizes || {}).map(([label, price]) => (
                        <button key={label} onClick={() => setSize(label)} style={{
                          ...
                          {label}
                          <div style={{ fontSize: 11, color: size === label ? `${BG}CC` : GOLD_DIM, marginTop: 2 }}>
                            {fmt(price)}
                          </div>
                        </button>
                      ))}
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {EXTRAS.map(({ key, label, price }) => (
                        <div key={key}
                          ...
                          <span style={{ color: GOLD_DIM, fontSize: 12 }}>+{fmt(price)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {TIMES.map(t => (
                  <button key={t} onClick={() => setTime(t)} style={{
                    padding: "12px",
                    background: time === t ? GOLD : "transparent",
                    border: `1px solid ${time === t ? GOLD : BORDER}`,
                    borderRadius: 2, cursor: "pointer",
                    color: time === t ? BG : TEXT_MUTED,
                    fontSize: 13,
                    fontFamily: "Cormorant Garamond, Georgia, serif",
                    transition: "all 0.2s",
                  }}>{t}</button>
            ...
            <div style={{
              ...
              {[
                ...
                <div key={label} style={{
                  ...
                  <span style={{ color: TEXT_MUTED, fontSize: 12, fontWeight: 600 }}>{label}</span>
                  <span style={{
                    color: String(label).startsWith("Deposit") ? GOLD : label === "Total" ? GOLD_LIGHT : TEXT,
                    fontSize: 13, fontWeight: String(label).startsWith("Deposit") ? 700 : 600,
                  }}>{value}</span>
                </div>
              ))}
            </div>
              ...
              <button onClick={async () => {
                ...
                try {
                  const res = await fetch("https://api.emailjs.com/api/v1.0/email/send", {
                    ...
                    body: JSON.stringify({
                      service_id: "service_q1ji004",
                      template_id: "template_04rp8lb",
                      user_id: "xGRvaLGUhrXZ_ESIp",
                      template_params: {
